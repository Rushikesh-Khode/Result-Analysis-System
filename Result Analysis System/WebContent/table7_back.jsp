...
else if(month<8)
{
	...
	int  ben=((cyear-2)*100000000);
	 ...
	 for(int i=0;i<3;i++)
	 {
		 ...
		 PreparedStatement pss1=con.prepareStatement("SELECT AVG(percent) FROM stud_data WHERE course=? AND Exam_Name =? AND percent>0 AND en_no>=? AND en_no <? AND year_cs=6 AND en_no IN (SELECT en_no FROM stud_data  WHERE course=? AND Exam_Name =? AND percent>0 AND en_no>=? AND en_no <? AND year_cs=5)");
		...
		rss.next();
			...
			PreparedStatement pss2=con.prepareStatement("SELECT count(percent) FROM stud_data WHERE course=? AND Exam_Name =? AND percent>0 AND en_no>=? AND en_no <? AND year_cs=6 AND en_no IN (SELECT en_no FROM stud_data  WHERE course=? AND Exam_Name =? AND percent>0 AND en_no>=? AND en_no <? AND year_cs=5)");
		 ...
		 PreparedStatement pss3=con.prepareStatement("SELECT count(*) FROM stud_data WHERE course=? AND Exam_Name =?  AND en_no>=? AND en_no <? AND year_cs=6 ");
